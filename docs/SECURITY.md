...
Leader results are not trusted: validators independently adjudicate the same canonical input. A malicious validator majority remains outside this contract's protection. Downstream consumers must use `is_valid`/`can_consume`; effective validity also checks bounded epoch and direct-parent revision bindings, so stale descendants fail closed without recursive writes. `PENDING`, `INVALID`, and `NEEDS_REVALIDATION` are unsafe. No privileged bypass exists. Network authenticity, source provenance, and domain-specific evidence quality remain application responsibilities.
